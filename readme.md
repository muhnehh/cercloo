# � Cercli HR Data Migration Platform
...
*Cercli: Transforming HR data chaos into compliance clarity.*

Start by fabricating 3 realistic but anonymized CSV files:

- `employee_master.csv` — messy columns like `emp_no`, `basic_sal`, `joining_dt`, `nationality_cd`, `visa_exp`
- `payroll_run.csv` — columns like `gross_pay`, `ot_hrs`, `ot_rate`, `housing_all`, `transport_all`
- `leave_records.csv` — `leave_type`, `days_taken`, `balance`, `yr_of_service`

Keep the column names intentionally messy — abbreviations, different conventions, Arabic transliterations. This makes the mapping problem real.

Your ingestion script does:

1. Load each file with pandas
2. Profile it: column names, inferred dtypes, null rates, min/max for numeric cols, sample values for string cols
3. Send that profile to Claude (via the Anthropic API) asking it to map each column to your canonical schema

The canonical schema you design — call it the "Cercli data model" — should have tables like `employees`, `contracts`, `payroll_runs`, `leave_balances`. Model it roughly after how a real HRIS stores data. Look at how Workday or BambooHR structure their exports for inspiration.

**Key skill you'll demonstrate:** Using an LLM not as a chatbot but as a structured data processing tool — you're calling the API programmatically, asking for JSON output, parsing it, and using confidence scores to flag uncertain mappings for human review.

---

### **Phase 2 — The compliance RAG layer (the most impressive part)**

**What to learn:** LangChain or LlamaIndex, vector stores (ChromaDB or FAISS), basic graph concepts

Build a corpus of UAE and KSA labor law. Your sources:

- UAE Federal Labour Law (Federal Decree-Law No. 33 of 2021) — available as PDF from MOHRE's website
- KSA Labour Law (Royal Decree M/51) — available in English translation
- Write short plain-English summaries of key articles yourself (annual leave minimums, overtime multipliers, notice periods, end-of-service gratuity rules)

For the RAG layer, implement two retrieval strategies so you can compare them:

**Baseline:** Simple vector search. Chunk the law text, embed with `text-embedding-3-small`, store in ChromaDB. Query with the compliance check question.

**Advanced (GraphRAG-lite):** Extract entities from the law text (contract types, leave types, thresholds like "30 days", "1.25x rate") and build a simple graph using NetworkX. Nodes are concepts; edges are relationships like "annual_leave MINIMUM 30 days FOR tenure_over_5_years". When you run a check, you first look up the relevant node in the graph, then use the vector store to pull the exact article text for the citation. This is your "GraphRAG" — you don't need a full knowledge graph library, just NetworkX plus a JSON file of relationships you populate manually.

This dual-retrieval approach is what separates this from a basic RAG demo. You can measure and compare them.

---

### **Phase 3 — The compliance checks**

With the mapped data and the RAG layer ready, implement these specific rules:

- Annual leave: UAE law mandates 30 days/year after 1 year of service, 2 days/month in the first year — flag anyone below this
- Overtime: Must be paid at 1.25x for weekday overtime, 1.5x for Friday/holiday — flag any payroll rows below this
- End-of-service gratuity: 21 days per year for first 5 years, 30 days/year thereafter — calculate and flag discrepancies
- Missing required fields: visa type, national ID, passport expiry, work permit number — these are legal requirements in UAE
- Probation period: Max 6 months under UAE law — flag any contracts with longer probation

For each violation, generate: the employee affected, the rule broken, the severity (warning vs error), and pull the relevant article text from your RAG layer with a confidence score.

---

### **Phase 4 — Evaluation (makes it credible)**

Create a small gold standard:

- 50 column mappings you manually labeled as correct, to measure mapping accuracy
- 20 synthetic employee records with known violations to measure precision/recall on compliance checks
- A timing benchmark comparing baseline vector RAG vs your GraphRAG-lite for retrieval latency and token usage

Put this in a Jupyter notebook called `evaluation.ipynb`. This is what tells Cercli you think like an engineer, not just a demo builder.

---

### **Phase 5 — The Streamlit UI**

Keep it simple. Three tabs:

1. **Upload** — drag and drop a CSV
2. **Mappings** — table showing `original_column | proposed_mapping | confidence | edit`
3. **Compliance** — list of warnings with severity badges, article citations, and expandable details

Use `st.data_editor` for the mapping table so users can correct the AI's suggestions. That one feature directly mirrors what Cercli would actually need — human-in-the-loop correction of automated mappings.
...
`python 3.11
pandas 2.x
anthropic (official Python SDK) — for column mapping LLM calls
langchain-community + chromadb — vector store
networkx — for the graph layer
streamlit — for the web UI
openpyxl — for Excel support
pytest — for your test suite`

Don't use OpenAI — use Anthropic's API since that's Cercli's likely stack preference and it's a nice touch.

---

### **Repo structure**

`cercli-migration-copilot/
├── README.md                  ← architecture diagram + demo GIF
├── data/
│   ├── samples/               ← your 3 fabricated CSVs
│   └── gold_standard/         ← evaluation labels
├── corpus/
│   ├── uae_labor_law.txt
│   └── ksa_labor_law.txt
├── src/
│   ├── ingestion.py           ← pandas profiling
│   ├── mapper.py              ← LLM-based column mapping
...
### **How to pitch this to the founder**

Here's your one paragraph, updated from the version in the brief to feel more personal and specific:

> "You told me to come back with something real. I built a MENA HR Migration Copilot — it takes the exact kind of messy employee and payroll CSVs companies hand over during onboarding, auto-maps them into a Cercli-style schema using Claude as a structured reasoning layer, and then runs UAE and KSA labor law compliance checks against a dual-retrieval RAG system I built over actual MOHRE regulations. The evaluation notebook shows mapping accuracy, compliance precision/recall, and a latency comparison between baseline vector search and my GraphRAG approach. I'd love to show you a 10-minute demo."
> 

---

### **What this demonstrates to them**

Cercli's hardest operational problem right now is customer onboarding — taking a new company's messy HR data and getting it into their system quickly and correctly. You're building the exact tool that would solve that. The compliance layer also maps to something a MENA HR platform absolutely needs and is hard to get right. Both pieces show you understand their domain, not just that you can code.

---

### **Where to start this week**

1. Build the 3 sample CSV files (1 hour) — make them realistic, include intentional errors
2. Write the pandas profiler and test it on your CSVs (2-3 hours)
3. Get the Anthropic API working and build the column mapper (3-4 hours)
4. Design the canonical schema in code (1-2 hours)

Once those four things work end-to-end, you have something you can already show. The RAG layer and compliance checks come after. Start with a CLI, add the Streamlit UI last.

Want me to write the actual starter code for any of these phases — the column mapper, the canonical schema definition, or the compliance rules engine?

To really impress the founder of a company like Cercli, I'd recommend a "hybrid" approach. Instead of just one dataset, you'd use one robust, widely-respected dataset as your primary testbed, and then showcase your deep regional understanding by augmenting it with real-world, local data. Here are**pasted**

choose the csv and igve me the csv file needed to make this project get goingand move on to the next step

12:41 PM

Let me create all the sample data files you need right now.

Created 5 files, ran 2 commands, read a file

Created 5 files, ran 2 commands, read a file

Create realistic messy HR CSV files

Script

Messy employee master CSV - realistic MENA HR export with abbreviations and inconsistencies

employee_master.csv

Messy payroll run CSV - realistic MENA payroll export with overtime violations and allowance issues

payroll_run.csv

Messy leave records CSV with entitlement violations and inconsistencies

leave_records.csv

Gold standard evaluation labels for column mapping accuracy testing

mapping_labels.csv

Gold standard compliance violations for evaluation - known pass/fail cases
...
All 5 files are ready. Here's what you have and why each decision was made deliberately:

**The 3 sample files** (your "messy legacy HR export"):

`employee_master.csv` — 20 employees across UAE, KSA, and mixed nationalities. Columns are intentionally abbreviated (`emp_nm`, `basic_sal`, `joining_dt`, `visa_exp`) and dates are in 4 different formats (`01/03/2019`, `15-Jul-2021`, `2020/01/10`, `05.08.2022`) — this is exactly what real HR exports look like. Some employees are missing IBANs, some have expired visas, one has no bank account at all.

`payroll_run.csv` — 2 months of payroll. Three employees (EMP003, EMP011, EMP018) have overtime rates below the UAE legal minimum of 1.25x. This is intentional — these are your "known violations" that the compliance engine must catch.

`leave_records.csv` — includes a maternity leave case (60 days, legally correct), a carry-forward that exceeds limits, first-year employees taking leave they haven't accrued yet, and an expired balance case.

**The 2 gold standard files** (your evaluation ground truth):

`mapping_labels.csv` — 65 labeled column mappings across all 3 files. Every column has its correct canonical name, the table it belongs to, data type, whether it's required, and whether it's compliance-relevant. This is what you measure your LLM mapper against.

`compliance_violations.csv` — 25 labeled test cases mixing true violations, near-misses, and compliant records. Each has the law article reference and whether `should_flag` is true or false. This lets you compute precision and recall.

**Next step:** Write `src/ingestion.py` — the pandas profiler that reads these CSVs, infers types, and prepares a summary to send to the Anthropic API for column mapping. Want me to build that now?
